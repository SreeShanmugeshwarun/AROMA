...
class YogaAssistant:
    def __init__(self, model_path="yoga_model.h5", confidence_threshold=0.7, mode="assist", camera_id=0):
        ...
        # Add AR-related attributes
        self.ar_mode = False
        self.ar_pose = None
        self.ar_models = {
            'warrior2': 'models/warrior2.glb',
            'tree': 'models/treepose.glb',
            'plank': 'models/plank.glb',
            'goddess': 'models/goddess.glb',
            'downdog': 'models/downdog.glb'
        }
    ...
    def process_frame(self, frame, display_frame):
        ...
        # Add AR overlay if AR mode is enabled
        if self.ar_mode and self.ar_pose:
            display_frame = self.add_ar_overlay(display_frame)
    ...
    def add_ar_overlay(self, frame):
        """Add AR pose overlay to the frame"""
        if not self.ar_pose:
            return frame
            
        # Draw AR guidance box
        overlay = frame.copy()
        height, width = frame.shape[:2]
        
        # Create semi-transparent overlay for AR info
        cv2.rectangle(overlay, (0, 0), (300, 80), 
                     self.colors["black"], -1)
        cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)
        
        # Show AR guidance text
        cv2.putText(frame, f"AR Guide: {self.ar_pose.capitalize()}", 
                    (10, 30), self.font, self.font_scale, 
                    self.colors["white"], self.line_type)
        cv2.putText(frame, "Press 'A' to exit AR mode", 
                    (10, 60), self.font, 0.6, 
                    self.colors["yellow"], 1)
        
        return frame

    def toggle_ar_mode(self, pose_name):
        """Toggle AR mode for a specific pose"""
        if self.ar_mode and self.ar_pose == pose_name:
            self.ar_mode = False
            self.ar_pose = None
        else:
            self.ar_mode = True
            self.ar_pose = pose_name
            print(f"AR mode enabled for {pose_name} pose")
