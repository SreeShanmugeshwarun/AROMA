...
class StreamlitYogaApp:
    def __init__(self):
        ...
        # Add AR models dictionary
        self.ar_models = {
            'warrior2': {'glb': 'models/warrior2.glb', 'scale': 1.0},
            'tree': {'glb': 'models/treepose.glb', 'scale': 1.0},
            'plank': {'glb': 'models/plank.glb', 'scale': 1.0},
            'goddess': {'glb': 'models/goddess.glb', 'scale': 1.0},
            'downdog': {'glb': 'models/downdog.glb', 'scale': 1.0}
        }
        ...
        # Initialize AR state in session
        if 'ar_active' not in st.session_state:
            st.session_state.ar_active = False
            st.session_state.current_ar_pose = None
        
        # Create sidebar with clickable pose names
        with st.sidebar:
            st.divider()
            st.subheader("📱 AR Pose Guides")
            for pose in ['warrior2', 'tree', 'plank', 'goddess', 'downdog']:
                if st.button(f"View {pose.capitalize()} Pose", key=f"ar_{pose}"):
                    st.session_state.ar_active = True
                    st.session_state.current_ar_pose = pose
    ...
    def process_frame(self, frame, display_frame):
        ...
        if st.session_state.ar_active and st.session_state.current_ar_pose:
            # Create columns for AR controls
            ar_col1, ar_col2 = st.columns([3, 1])
            
            with ar_col1:
                st.markdown(f"### AR Guide: {st.session_state.current_ar_pose.capitalize()} Pose")
            
            with ar_col2:
                if st.button("❌ Close AR", key="close_ar"):
                    st.session_state.ar_active = False
                    st.session_state.current_ar_pose = None
                    st.experimental_rerun()
            
            # Show AR view below camera feed
            ar_html = self.create_ar_viewer_html(st.session_state.current_ar_pose)
            components.html(ar_html, height=400)
        
        return display_frame

    def create_ar_viewer_html(self, pose_name):
        """Create HTML for AR viewer"""
        ar_html = f"""
        <html>
        <head>
            <title>Yoga Pose AR Guide</title>
            <script src="https://aframe.io/releases/1.2.0/aframe.min.js"></script>
            <script src="https://raw.githack.com/AR-js-org/AR.js/master/aframe/build/aframe-ar.js"></script>
            <style>
                .ar-container {{ height: 400px; width: 100%; }}
            </style>
        </head>
        <body style="margin: 0; overflow: hidden;">
            <div class="ar-container">
                <a-scene embedded arjs="sourceType: webcam; debugUIEnabled: false;">
                    <a-assets>
                        <a-asset-item id="pose-model" src="{self.ar_models[pose_name]['glb']}"></a-asset-item>
                    </a-assets>
                    ...
                    <a-marker preset="hiro">
                        <a-entity 
                            position="0 0 0"
                            rotation="0 0 0"
                            scale="{self.ar_models[pose_name]['scale']} {self.ar_models[pose_name]['scale']} {self.ar_models[pose_name]['scale']}"
                            gltf-model="#pose-model">
                        </a-entity>
                    </a-marker>
    ...
    def handle_pose_button(self, pose_name):
        """Handle pose button click in sidebar"""
        if st.button(f"View {pose_name.capitalize()} Pose", key=f"pose_{pose_name}"):
            self.assistant.toggle_ar_mode(pose_name)
